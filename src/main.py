from experimental_data.experiment_results import draw_experiment_result, evaluate_new_points, parse_points
...
def generate_experiment_screen(root):
    # Función para la pantalla de generación de experimentos
    def run_algorithm():
        ...
        # Generar experimento y obtener puntos
...
def evaluate_points_screen(root):
    ...
    points_entry = ttk.Entry(evaluate_window, width=50)  # Ancho aumentado
...
def main():
    def open_generate_experiment():
        generate_experiment_screen(root)
    def open_run_algorithm():
        run_algorithm(root)
    def open_revisit_experiment():
        revisit_experiment_screen(root)
    def open_evaluate_points():
        evaluate_points_screen(root)
